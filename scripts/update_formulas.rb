...
UPSTREAM_REPO = ENV.fetch("UPSTREAM_REPO", "atusy/kakehashi")
...
# Map from release target triple to the JSON keys used in versions.json.
TARGET_TO_JSON = {
...
def parse_semver(tag)
  return nil unless tag =~ /\Av(\d+\.\d+\.\d+)\z/
...
# Convert release_info output to a versions.json entry hash.
...
# Build the target state of versions.json from upstream releases.
# Only adds keys that already exist in current_json or match new_main_mm.
def build_versions_json(current_json, latest_per_mm, overall_latest_entry, new_main_mm, report)
  result = {}

  # Main formula → overall latest.
  ...
  latest_per_mm.each do |(major, minor), entry|
    key = "#{major}.#{minor}"
    next if !current_json.key?(key) && [major, minor] != new_main_mm
...
def run_updater
  ...
  latest_per_mm = parsed
    ...
    .each_with_object({}) { |(k, v), h| h[k] = v.max_by { |e| e[:version] } }
  ...
  # Ensure .rb files exist for any new keys.
...
def self_test
  ...
  class_cases.each do |(major, minor), expected|
    ...
    failures << "class_name_for(#{major}, #{minor}) => #{got.inspect}, expected #{expected.inspect}" if got != expected
  end
...
if $PROGRAM_NAME == __FILE__
  ...
  when "--self-test"
    self_test
  when "--probe"
    tag = ARGV[1] or (warn "usage: #{$PROGRAM_NAME} --probe <tag>"; exit 2)
    probe(tag)
  when nil
    run_updater
  else
